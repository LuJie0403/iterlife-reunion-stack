...
本文档记录 `iterlife-stack` 当前仍在使用的 GitHub Actions secrets，以及这些 secret 分别属于哪个仓库和哪条流水线。

## 1. `iterlife-stack` 仓库自身 Secret
...
这两个 secret 由调用该 workflow 的业务仓库提供，不配置在 `iterlife-stack` 仓库自身。
...
- 若某个 secret 只属于业务仓库，不应误写成 `iterlife-stack` 的自有 secret。
